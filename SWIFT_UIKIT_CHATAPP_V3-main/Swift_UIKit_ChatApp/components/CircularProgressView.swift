...
// Function to start circling the progress
func startCircling() {
    // Cancel the existing timer if it's already running
    progressTimer?.invalidate()
    
    var newProgress: Float = 0.0
    // Start a new timer with a time interval of 0.05 seconds
    progressTimer = Timer.scheduledTimer(withTimeInterval: 0.005, repeats: true) { [weak self] timer in
        // Calculate the new progress value (increase by a small value)
        
        
        newProgress += 0.005
    
        
//        self?.setProgress(duration: timeToFill, to: progress)

        // Check if the progress has reached 100% (1.0), then reset it to 0.0
        if newProgress >= 1.0 {
            self?.progress = 0.0
            newProgress = 0
        } else {
            // Update the progress
            self?.setProgress(to: newProgress)
        }
    }
}
    
func stopCircling() {
    // Invalidate the timer to stop updating the progress
    progressTimer?.invalidate()
    progressTimer = nil
}
