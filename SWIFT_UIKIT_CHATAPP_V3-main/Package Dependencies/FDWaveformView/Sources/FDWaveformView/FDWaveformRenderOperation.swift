...
final public class FDWaveformRenderOperation: Operation {
    ...
    var directionStyle: DirectionStyle = .topAndBottom {
        didSet {
        }
    }
...
//        context.setLineWidth(0.2 / format.scale)
        ...
        var sampleDrawingScale: CGFloat!
        if max == min {
        ...
        while x < samples.count {
           let sample = samples[x]
            ...
            let default_sample_width = screen_width * 0.02335
            

            if sample >= -100 && sample <= -35 {
                context.move(to: CGPoint(x: CGFloat(x), y: verticalMiddle - (default_sample_width)))
                context.addLine(to: CGPoint(x: CGFloat(x), y: verticalMiddle + (default_sample_width)))
            } else {
                
                switch directionStyle {
                    case .onlyTop:
                        context.move(to: CGPoint(x: CGFloat(x), y: verticalMiddle - (height / 1.6)))
                        context.addLine(to: CGPoint(x: CGFloat(x), y: verticalMiddle + (0)))
                    case .onlyBottom:
                        context.move(to: CGPoint(x: CGFloat(x), y: verticalMiddle - (0)))
                        context.addLine(to: CGPoint(x: CGFloat(x), y: verticalMiddle + (height / 1.6)))
                    case .topAndBottom:
                        context.move(to: CGPoint(x: CGFloat(x), y: verticalMiddle - (height / 1.6)))
                        context.addLine(to: CGPoint(x: CGFloat(x), y: verticalMiddle + (height / 1.6)))
                }
           ...
           context.setLineWidth(CGFloat(20)) // note: stroke width straddles the path, so play with this
           context.strokePath()
