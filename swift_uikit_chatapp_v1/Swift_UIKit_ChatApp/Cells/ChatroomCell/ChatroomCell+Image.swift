//
//  ChatroomCell+image.swift
//  Swift_UIKit_ChatApp
//
//  Created by Al-Tameemi Hayder on 11.7.2023.
//
...
//        messageImageLeading = messageImage.leadingAnchor.constraint(equalTo: bubbleView.leadingAnchor, constant: 2)
//        messageImageTrailing = messageImage.trailingAnchor.constraint(equalTo: bubbleView.trailingAnchor, constant: -2)
        
        messageImageBottomToBubbleView = messageImage.bottomAnchor.constraint(equalTo: bubbleView.bottomAnchor, constant: -2)
        
        messageImageBottomToTextMessage = messageImage.bottomAnchor.constraint(equalTo: messageTextLabel.topAnchor, constant: -5)
...
//    func fixImageSize() {
//        if let image = messageImage.image {
//
////            let manipulatedHeight = image.size.height
////            let manipulatedWidth = image.size.width
////            messageImage.widthAnchor.constraint(equalTo: messageImage.heightAnchor, multiplier: manipulatedWidth / manipulatedHeight).isActive = true
//
//
//            let manipulatedWidth = messageImage.frame.width * 1
//            messageImageWidthConstraint = messageImage.widthAnchor.constraint(equalToConstant: manipulatedWidth)
//            messageImageWidthConstraint?.isActive = true
//
//
//
//            let ratio = manipulatedWidth / image.size.width
//            let scaledHeight = image.size.height * ratio
//
//
//            messageImageHeightConstraint = messageImage.heightAnchor.constraint(equalToConstant: scaledHeight)
                            ...
                            DispatchQueue.main.async {
                                ...
                                if progress == 1.0 {
                                    ...
                                    // when its image
                                        ...
                                        self?.updateMessage((isVideo: false, isPhoto: true))
                                        completion(true)
                                    }
...
//        print("no image url connection error: ", customProgressView.alpha)
    ...
    func showVideoAnimation() {
        ...
        self.hideBlur()
        self.downloadImageVideoView.showHideIndicator(show: false)
        self.updateMessage((isVideo: true, isPhoto: false))
